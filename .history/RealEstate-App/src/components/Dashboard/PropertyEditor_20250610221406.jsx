...
const validationSchema = Yup.object().shape({
  ...
  installment_price_per_month: Yup.number().when("installment_available", {
    is: true,
    then: (schema) => schema.required("Installment price is required"),
  }),
  installment_period_months: Yup.number().when("installment_available", {
    is: true,
    then: (schema) => schema.required("Installment period is required"),
  }),
  description: Yup.string().trim().required("Description is required"),
...
const PropertyEditor = () => {
  ...
  const location = useLocation();
  const { propertyToEdit } = location.state || {};
  const { allProperties, updateProperty, addProperty } =
